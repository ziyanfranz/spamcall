...
try:
	...
	exit(
		f"\n \x1b[97m[\x1b[95m!\x1b[97m] Mendonlot:v install dulu module requests nya\n     Ketik \x1b[90m: \x1b[93mpip install requests\n\x1b[97m"
	)
	...
	def Main(
		...
		try:
			__import__(
				...
				f"\n {white}[{purple}◈{white}] Contoh {black}: {white}812xxx"
		)
			self.phone = str(
					input(
			...
			if self.phone in(
					...
					f" {green}[ {white}Warning {red}! {white}Isi Nomernya Hadeh:V {green}]{white}\n"
			)
				...
				print(
					f"\n {purple}[ {white}Warning {red}! {white}Harap input no dengan angka awalan 8 :V {purple}]{white}\n"
			)
